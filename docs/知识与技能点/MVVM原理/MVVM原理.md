...
当`set`修改值的时候执行了`dep.notify`方法，这个方法是执行`watcher`的`update`方法，那么我们再对`update`进行修改一下
...
`reduce`的用处多多，比如计算数组求和是比较普通的方法了，还有一种比较好用的妙处是可以进行二维数组的展平(flatten)，各位不妨来看最后一眼
